...
class categoryClass:
    def __init__(self,root):
        ...
        self.category_table=ttk.Treeview(cat_frame,columns=("C_ID","name"),yscrollcommand=scrolly.set,xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM,fill=X)
        scrolly.pack(side=RIGHT,fill=Y)
        scrollx.config(command=self.category_table.xview)
        scrolly.config(command=self.category_table.yview)

        self.category_table.heading("C_ID",text="C_ID")
        ...
        #===IMAGES============
        self.im1=Image.open("inventory images/cat.jpg")
        ...
        self.im2=Image.open("inventory images/category.jpg")
    ...
    def delete(self):
        ...
        try:
            if self.var_cat_id.get()=="":
                ...
                cur.execute("select * from category where C_ID=? ",(self.var_cat_id.get(),))
                row=cur.fetchone()
                if row==None:
                    ...
                    if op==True:
                        cur.execute("delete from category where C_ID=?",(self.var_cat_id.get(),))
                        con.commit()
